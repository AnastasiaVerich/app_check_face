...
export const useFaceDetection = (
    ...
    // Детекция лиц
    useEffect(() => {
        let timeoutId: any = null;        // Если модели загружены и камера включена
        if (modelsLoaded && isCameraOn && isLoaded) {
            ...
            const detectFace = async () => {
                ...
                try {
                    ...
                    // Запускаем следующий вызов через 100 мс после завершения
                    timeoutId = setTimeout(detectFace, 100);
                }
            };
            detectFace()
        }
